...
static int                  EBPImmediateReturn = 0;
...

{
  ...
  //rhs is pntdim - 1
  //ReportMsg("\nLocating new solution point with FindPoint\n");
  ...
  for (iter = 0; iter < max_iter; iter++)
    ...
    // ReportMsg("\nIteration loop with iter %d\n", iter);
     ...
     //ReportMsg("\nCall of equation within FindPoint\n");
     ...
     //ReportMsg("\t Not Converged\n");
      ...
      if ((iter == 0) && JacImport)
        ...
        {
          ReportMsg("%-s", "Computing jacobian in FindPoint\n");
      ...
      //ReportMsg("\tNot Converged, new iter\n");
    }

  free(basemem);

  if (retcode == SUCCES){
      ReportMsg("New solution point found with FindPoint\n\n");
  }
  else {
                             ...
                             double *result, int fast)
{
  ...
  old =*x; // oldvalue of the solution.
  h   = h0; //change in the value
  for (i = 0; i < 2; i++)
    {
      *x = old + h; //new value
      if ((*fnc)(farg, frhs) == FAILURE) return FAILURE; //solve the new eq (farg = guess, frhs = new sol)
  ...
  *x = old; //set the element back to old value
...

{
  ...
  y = basemem = calloc(2*pntdim, sizeof(double)); //allocates memory and init to 0.twice the size of pntdim
  if (!basemem) return ReportMemError("Jacobian");
    
    rhs = y + pntdim; //rhs points to just after pnt


  // Initialize
  COPY(pntdim, pnt, 1, y, 1); //pnt is copied to y
  memset((void *)jac, 0, (pntdim*fncdim)*sizeof(double)); //sets the jac to zero

  for (j = 0; j < pntdim; j++)
    {
      ydif = fabs(Jacobian_Step*y[j]); //small difference in pnt
...
/*==================================================================================================================================*/
//NEW to locate where R0_yy = 0//
int FindEBP(const int pntdim, double *pnt,  int (*fnc)(double *, double *), double dytol, double rhstol, const int R0index, double *EVmaxH,  const int detecting)
...

{
  ...
  double  *basemem, *y, *tv, *rhs0, *rhs, *Jac, *pardif, *eigvecH,  *CEhes;
  const int ebppntdim = pntdim - 1;

 //  ReportMsg("Start EBP, when EBPCalc is %d\n", EBPCalc);
   
 //prevent recurrence
    *EVmaxH = 0;
    if (EBPImmediateReturn) return SUCCES;
    EBPImmediateReturn = 1;
 ...
 */
  ...
  if (!basemem) return ReportMemError("FindEBP");

  /*ReportMsg("\nTo continue EBP, Classify ESS point of %d parameter(s) of pnt:\t", evoParsDim_single);
  ...
  for (i = 1; i < (ebppntdim - evoParsDim); i++) //1 t/m 10
    memmove(Jac + i*(ebppntdim - evoParsDim - 1), Jac + i*(pntdim - 1), (ebppntdim - evoParsDim - 1)*sizeof(double));

  //ReportMsg("\nFor i 1 %d. Move %d to %d\t", i, 1*(pntdim - 1), (ebppntdim - //evoParsDim - 1));
  // Zero-out the remaining entries of the Jacobian
  memset((void *)(Jac + (ebppntdim - evoParsDim)*(ebppntdim - evoParsDim - 1)), 0, (pntdim*(pntdim - 1) - (ebppntdim - evoParsDim)*(ebppntdim - evoParsDim - 1))*sizeof(double));
  ...
  for (i = 0; i < evoparsdim_single; i++)
    {
      ...
      parPntr[indx1] = oldvalue1 + pardif[i];
       // ReportMsg("\nEBPC: The old value of parameter nr %d with index %d is %f. the new value is %f. Pop_evo is %d:\t", i,indx1, oldvalue1, parPntr[indx1], R0index);
  ...
  //ReportMsg("\n At end of calculation, CEhes is: %.10f and EBPCalc %d\n", *EVmaxH, EBPCalc);
  free(basemem);
  EBPImmediateReturn = 0;
