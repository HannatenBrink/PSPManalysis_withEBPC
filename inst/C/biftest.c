 ...
 */
{
  int     i, j, ii, jj, indx1, indx2, evoparsdim, evoparsdim_single;
 ...
 */
  evoparsdim = evoParsDim + detecting; ///dimension of evo parameters of all pops
  evoparsdim_single = evoParsDim_single + detecting; //dimension of evo parameters of the current pop for which the derivates are calculated 
    // Include Bifparone in the evolving parameter list if detecting
  ...
  ReportMsg("\nClassifying ESS point of %d parameter(s) of pnt:\t", evoParsDim_single, R0index);
  for (i = 0; i < pntdim; i++) ReportMsg("%10.5E\t", pnt[i]*pnt_scale[i]);
  ReportMsg("\n");

    
      
  tv      = y + pntdim; //tangent vector
  rhs0    = tv + pntdim;
  rhs     = rhs0 + pntdim;
  Jac     = rhs + pntdim;
  ...
  for (i = 0; i < evoparsdim_single; i++)
    {
      ...
      //ReportMsg("\nThe old value of parameter nr %d with index %d is %f:\t", i,indx1, oldvalue1);
      pardif[i]      = max(fabs(Jacobian_Step*parPntr[indx1]), Jacobian_Min_Step);
      parPntr[indx1] = oldvalue1 + pardif[i];
        ReportMsg("\nclassify: The old value of parameter nr %d with index %d is %f. the new value is %f. Pop_evo is %d:\t", i,indx1, oldvalue1, parPntr[indx1],R0index);
  ...
  for (i = 0; i < evoparsdim_single; i++)
    {
       ...
       */
      CEhes[i*evoparsdim_single + i] = -2*(rhs0[R0index] + 1.0);
      ...
      for (ii = 0; ii < 2; ii++)
        {
          ...
          CEhes[i*evoparsdim_single + i] += (rhs[R0index] + 1.0);
              ...
              for (jj = 0; jj < 2; jj++)
                {
                  ...
                  CEhes[i*evoparsdim_single + j] += (1 - 2*ii)*(1 - 2*jj)*(rhs[R0index] + 1.0);
                }
      ...
      CEhes[i*evoparsdim_single + i] /= pardif[i]*pardif[i];                               // This like in the 1-dimensional case
      for (j = 0; j < i; j++)
        {
          CEhes[i*evoparsdim_single + j] /= 4*pardif[i]*pardif[j];
          CEhes[j*evoparsdim_single + i] = CEhes[i*evoparsdim_single + j];                        // Hessian is symmetric
        }
    }
  ...
  for (i = 0; i < evoparsdim_single; i++)
    {
              ...
              for (jj = 0; jj < 2; jj++)
                {
                  ...
                  CEjac[i*evoparsdim_single + j] += (1 - 2*ii)*(1 - 2*jj)*(rhs[R0index] + 1.0);
                }
              parPntr[indx2] = oldvalue2;                                           // Reset parameter
            }
              ...
              for (jj = 0; jj < 2; jj++)
                {
                  ...
                  // The factor (1-2*ii)*(1-2*jj) gives the appropriate sign in the expression (**) above
                  CEC01[i*evoparsdim_single + j] += (1 - 2*ii)*(1 - 2*jj)*(rhs[R0index] + 1.0);
                }
              parPntr[indx2] = oldvalue2;                                           // Reset parameter
            }
      ...
      for (j = 0; j < evoparsdim_single; j++)
        {
#if (COMPUTEJACOBIAN == 1)
          CEjac[i*evoparsdim_single + j] /= 4*pardif[i]*pardif[j];
#endif

#if (COMPUTECROSS == 1)
          CEC01[i*evoparsdim_single + j] /= 4*pardif[i]*pardif[j];
#endif
#if (COMPUTECROSS == 0)
          CEC01[i*evoparsdim_single + j] = CEjac[i*evoparsdim_single + j] - CEhes[i*evoparsdim_single + j];
#endif

#if (COMPUTEJACOBIAN == 0)
          CEjac[i*evoparsdim_single + j] = CEhes[i*evoparsdim_single + j] + CEC01[i*evoparsdim_single + j];
#endif
#if (COMPUTEHESSIAN == 0)
          CEhes[i*evoparsdim_single + j] = CEjac[i*evoparsdim_single + j] - CEC01[i*evoparsdim_single + j];
#endif
        }
    }

  if ((evoparsdim_single== 1) && !detecting)
    {
    ...
    {
      Eigenval(evoparsdim_single, CEhes, 1, EVmaxH, DOMINANT, eigvecH, NULL, rhstol);
      Eigenval(evoparsdim_single, CEjac, 0, EVmaxJ, DOMINANT, eigvecJ, NULL, rhstol);
      ...
      if (EVmaxJs)
        {
          for (i = 0; i < evoparsdim_single; i++)
            for (j = 0; j < evoparsdim_single; j++) CEjacS[i*evoparsdim_single + j] = (CEjac[i*evoparsdim_single + j] + CEjac[j*evoparsdim_single + i])/2;
              Eigenval(evoparsdim_single, CEjacS, 1, EVmaxJs, DOMINANT, eigvecJ, NULL, rhstol);    // We do not need the eigenvector of the Jacobiaan, re-use it
        }
      ...
      for (i = 0; i < evoparsdim_single; i++)
        {
          ...
          */
          ...
          for (j = 0; j < evoparsdim_single; j++) eigvecJ[i] += CEC01[i*evoparsdim_single + j]*eigvecH[j];
        }
      // Compute Z^T C_01 Z
      *zC01z = 0.0;
      for (j = 0; j < evoparsdim_single; j++) *zC01z += eigvecJ[j]*eigvecH[j];
    }
  setBifParVal = 1;
  CurveType    = OldCurveType;                                                      // Reset the curve type
...
//NEW
int ESSclassify2(const int pntdim, double *pnt,  int (*fnc)(double *, double *), double dytol, double rhstol, const int R0index, double *EVmaxJ,
                double *EVmaxJs, double *EVmaxH, double *zC01z, const int detecting)

/*
 * ESSclassify2 -  Routine assess the stability properties of an ESS point by computing
 *                the second derivatives of R0(E(x),y) w.r.t. life history parameters, both
 *                the resident and mutant parameter value.
 * Not r0_yy?
 *
 ...
 */
  ...
  ReportMsg("\nClassifying (2) ESS point of %d parameter of pnt:\t", evoParsDim_single, R0index);
  for (i = 0; i < pntdim; i++) ReportMsg("%10.5E\t", pnt[i]*pnt_scale[i]);
  ReportMsg("\n");
  ...
  // Compute the Jacobian for general use in FindPoint() in the loop below
    ReportMsg("\nJacobian is calculated for general use\t");
     Jacobian(pntdim, y, pntdim-1, Jac, fnc, FORWARD);
    ReportMsg("\End of Jacobian\t");
    
  // FindPoint() below only needs Jacobian of system without evolutionary parameters
  // Move appropriate elements of Jacobian into right place, overwriting derivatives w.r.t. evolutionary parameters
  // memmove() used instead of memcpy() because src and dest overlap (undefined behavior in memcpy)
    
    //maybe also get rid of the second par?
    //pntdim = 1 + 7*env + 3*birth + par2 + 3*traits = 15
    // evoParsDim = 3
    //fncdim = 14
    //jac is dan 15x14 (rxc)
    //I would guess that only the last parts are removed? and you would get 12x14 (and should become 11x14)
    //Isn't it that the equations are removed? so the ones for the ess calc?
    
    for (i = 1; i < (pntdim - evoParsDim); i++)
      memmove(Jac + i*(pntdim - evoParsDim - 1), Jac + i*(pntdim - 1), (pntdim - evoParsDim - 1)*sizeof(double));
    //jac is 15x14 of which maybe you want to make 12x14
    //i 1 tm 12
    //i = 1
    //Jac+11 = dest
    //Jac+14 = source
  ...
  // Zero-out the remaining entries of the Jacobian
    memset((void *)(Jac + (pntdim - evoParsDim)*(pntdim - evoParsDim - 1)), 0, (pntdim*(ebppntdim - 1) - (pntdim - evoParsDim)*(pntdim - evoParsDim - 1))*sizeof(double));
    
   
  // Function call to set all parameters to their appropriate values, preserve these values in rhs0
  //First a normal function//
    ReportMsg("\nA normal calculation is done with the point\t");
  if ((*fnc)(y, rhs0) == FAILURE) //why?
    {
      ErrorMsg(__FILE__, __LINE__, "Right-hand side computation failed");
      free(basemem);
      return FAILURE;
  ...
  for (i = 0; i < evoparsdim_single; i++)
    {
      if (detecting && !i)
        ...
        indx1 = evoParsIndexPntr2[i - detecting];                                    // Runs to i=0..evoParsDim in detecting and non-detecting case
      oldvalue1      = parPntr[indx1];
      
      pardif[i]      = max(fabs(1.0E-4*parPntr[indx1]), Jacobian_Min_Step); //somehow the pardif is too big for this to work. But, it does work for a normal ess calculation, so why not now? should I also get rid of the secondbifpar?
      ...
      for (ii = 0; ii < 2; ii++)
        {
          if (ii == 0)
            ...
            parPntr[indx1] = oldvalue1 - pardif[i];
          ReportMsg("\nA new calculation is done (L1250), where parameter %d now equals %f (before %f)\t",indx1, parPntr[indx1],oldvalue1 );
          if ((*fnc)(y, rhs) == FAILURE)
            {
              ErrorMsg(__FILE__, __LINE__, "Right-hand side computation failed");
              setBifParVal = 1;
          ...
          for (j = 0; j < i; j++)
            {
              if (detecting && !j)
                ...
                indx2 = evoParsIndexPntr2[j - detecting];                            // Runs to i=0..evoParsDim in detecting and non-detecting case
              ...
              for (jj = 0; jj < 2; jj++)
                {
                  if (jj == 0)
                    ...
                    parPntr[indx2] = oldvalue2 - pardif[j];
                ReportMsg("\nA new calculation is done (L1276), where parameter %d now equals %f (before %f)\t",indx2, parPntr[indx2],oldvalue2 );
                  if ((*fnc)(y, rhs) == FAILURE)
                    {
                      ErrorMsg(__FILE__, __LINE__, "Right-hand side computation failed");
                      setBifParVal = 1;
      ...
      for (ii = 0; ii < 2; ii++)
        {
          ...
          // Initialize anew
          COPY(pntdim, pnt, 1, y, 1);
          //  COPY(ebppntdim - evoParsDim, pnt, 1, y, 1); //new?
            
            ReportMsg("\nA new equi calculation is done (L1325), where parameter %d now equals %f (before %f)\t",indx1, parPntr[indx1],oldvalue1 );
          retval = FindPoint(ebppntdim - evoParsDim, y, Jac, tv, dytol, rhstol, MAXITER, fnc); //one dim less because 2 parameters //Here things go wrong but why? With smaller steps it works better but still.
          if (retval != SUCCES)
            {
              ErrorMsg(__FILE__, __LINE__, "Location of fixed point for 2nd derivatives failed");
              setBifParVal = 1;
          ...
          for (j = 1; j < (ebppntdim - evoParsDim); j++)
            if (y[j] != pnt[j]) break;
          if (j == (pntdim - evoParsDim))
            ErrorMsg(__FILE__, __LINE__, "ESS classification possibly inaccurate. Derivative of environment undetermined (E(x*+dx)=E(x*))\n");
          /*
          ...
          for (j = 0; j < evoparsdim_single; j++)
            {
              ...
              oldvalue2 = parPntr[indx2];
                ReportMsg("\nA new equi calculation is done (L1366), where parameter %d now equals %f (before %f)\t",indx2, parPntr[indx2],oldvalue2 );
              for (jj = 0; jj < 2; jj++)
                {
                  if (jj == 0)
                    parPntr[indx2] = oldvalue2 + pardif[j];
              ...
              for (jj = 0; jj < 2; jj++)
                {
                  if (jj == 0)
                    ...
                    parPntr[indx2] = oldvalue2 - pardif[j];
                ReportMsg("\nA new equi calculation is done (L1419), where parameter %d now equals %f (before %f)\t",indx2, parPntr[indx2],oldvalue2 );
                  if ((*fnc)(y, rhs) == FAILURE)
                    {
                      ErrorMsg(__FILE__, __LINE__, "Right-hand side computation failed");
                      setBifParVal = 1;
