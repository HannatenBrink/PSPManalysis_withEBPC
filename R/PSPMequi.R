...
PSPMequi <- function(modelname = NULL, biftype = NULL, startpoint = NULL, stepsize = NULL, 
  ...
  if (!is.character(biftype)) stop('Bifurcation type should be a string (BP, BPE, EQ, LP, ESS, EBPC or PIP)')
  ...
  if (Rmodel == 1) {
    if ( (!length(parbnds)) || (!((length(parbnds) == 3) || (length(parbnds) == 6) || (((length(parbnds)-6) %% 4) == 0) || (((length(parbnds)-3) %% 4) == 0))) )
  ...
  if ((!is.double(parbnds)) || (!((length(parbnds) == 3) || (length(parbnds) == 6) || (((length(parbnds)-6) %% 4) == 0) || (((length(parbnds)-3) %% 4) == 0))))
    stop('Parameter bounds values should be a vector of length 3, 6 or 3+4*N (in case of ESS continuation)')
